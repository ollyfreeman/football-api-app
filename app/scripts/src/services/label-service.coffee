...
.factory('Labels', [ 'GRAPH', 'TIME', (GRAPH, TIME) ->
    ...
    for i in[TIME.FIRSTHALF_START..TIME.FIRSTHALF_END] by 1
        labelService.tooltipLabels[i] = "#{i} FH"
    for i in[TIME.HALFTIME_START..TIME.HALFTIME_END] by 1
        labelService.tooltipLabels[i] = "#{i-TIME.HALFTIME_START} HT"
    for i in[TIME.SECONDHALF_START..TIME.SECONDHALF_END] by 1
